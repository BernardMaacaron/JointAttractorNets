...
void _run_glob_inh_neuron_spike_thresholder_codeobject_1()
{
...
const size_t _numt = 1;
    ...
    int32_t* __restrict  _ptr_array_glob_inh_neuron__spikespace = _array_glob_inh_neuron__spikespace;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double V = _ptr_array_glob_inh_neuron_V[_idx];
        const char not_refractory = _ptr_array_glob_inh_neuron_not_refractory[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (V > Vth) && false;
        else 
        ...
        if(_cond) {
            _ptr_array_glob_inh_neuron__spikespace[_count++] = _idx;
            _ptr_array_glob_inh_neuron_not_refractory[_idx] = false;
            _ptr_array_glob_inh_neuron_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_glob_inh_neuron__spikespace[N] = _count;
