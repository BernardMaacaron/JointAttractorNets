...
double* const _array_glob_inh2pool_delay = _dynamic_array_glob_inh2pool_delay.empty()? 0 : &_dynamic_array_glob_inh2pool_delay[0];
const size_t _numdelay = _dynamic_array_glob_inh2pool_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_glob_inh_neuron__spikespace = _array_glob_inh_neuron__spikespace;
    double* __restrict  _ptr_array_glob_inh2pool_delay = _array_glob_inh2pool_delay;

    std::vector<double> &real_delays = _dynamic_array_glob_inh2pool_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = glob_inh2pool_pre.sources.empty() ? 0 : &(glob_inh2pool_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = glob_inh2pool_pre.sources.size();
    glob_inh2pool_pre.prepare(_n_sources,
                           _n_targets,
