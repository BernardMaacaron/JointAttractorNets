...
def load_trajectory_data(file_path):
    ...
    data['rounded_acceleration'] = data['acceleration'].apply(round_velocity)
...
def runSimulation(ringAttractor, device = None, dt=0.1*ms,
    ...
    if flag:
        ringAttractor.ring_pool.I_ext = I_ext_array
        # Default velocity input for asymmetrical synapses
...
def simulate_with_trajectory(data, alpha_value, beta_value=0.0, initial_position=None, duration=None):
    """
    Simulate the ring attractor using a trajectory from data file with both velocity and acceleration.
    
    Args:
        data: DataFrame with time, position, velocity, acceleration columns
        alpha_value: Alpha parameter for velocity
        beta_value: Beta parameter for acceleration
...
def train_from_trajectory_files(folder_path="/home/fferrari-iit.local/RingAttractor/Ring_Attractor_madeByMe/capocaccia", output_dir="/home/fferrari-iit.local/RingAttractor/Ring_Attractor_madeByMe/Images/Mujoco_training_withoutSymmetry",duration=None):
    """
    Train g(v) function from trajectory files.
    ...
    # Find optimal alpha for each unique velocity using parallel processing
    ...
    with Pool(processes=cpu_count()) as pool:
        optimization_results = list(tqdm(
            pool.imap(optimize_alpha_beta_for_velocity, optimization_args),
    ...
    # Save velocity -> alpha mapping
    csv_path = os.path.join(output_dir, "velocity_alpha_mapping.csv")
    velocity_data.to_csv(csv_path, index=False)
    print("Velocity to alpha mapping saved to velocity_alpha_mapping.csv")
    
   
    plt.figure(figsize=(12, 10))
    # First subplot: g(v) vs velocity
    plt.subplot(2, 1, 1)
    ...
    # Second subplot: v*g(v) vs velocity
    plt.subplot(2, 1, 2)
    # Calculate the product of velocity and optimal_alpha
...
def validate_trajectory_with_gv(data_file, velocity_data):
    ...
    for data_idx in range(duration):
        ...
        # Get velocity and position from data
        velocity = data['velocity'].iloc[data_idx]
        ...
        if rounded_velocity in velocity_data['velocity'].values:
            alpha = velocity_data.loc[
                velocity_data['velocity'] == rounded_velocity, 'optimal_alpha'
            ].iloc[0]
        else:
            # Find closest velocity
            closest_idx = (velocity_data['velocity'] - rounded_velocity).abs().idxmin()
            alpha = velocity_data.loc[closest_idx, 'optimal_alpha']
        
        # Setup input parameters
        dt = 0.1*ms
        inputParams = {'I0': 22.00, 'targetPosition': current_position, 'I_target': 1.0}
        velocityInput = alpha * velocity / 1000.0  # Convert deg/s to deg/ms
