...
class RingAttractor():
    def __init__(self, N =120, Vth=-48 * mV, V_rest=-70 *mV, V_reset=-80 *mV, wee = 46*mV, wei = 46*mV, wie = 420 *mV, wii = 560 *mV, sigma=0.4, mode_weights="gaussian"):
        #self.Vthr = Vth * mV
        ...
        self.inhibit_neuron = NeuronGroup(1, LIF_eq, threshold = "V > Vth", reset = "V = V_reset", method="euler", name="inhibitiry_neurons", dt=2*ms)
        self.excitatory_neurons = NeuronGroup(self.N, LIF_eq, threshold = "V > Vth", reset = "V = V_reset", method="euler", name="excitatory_neurons", dt=2*ms)
        ...
        self.Ring2Inh = Synapses(self.excitatory_neurons, self.inhibit_neuron, "W_ring2inh : volt", name="ring2inh_synapses", dt=2*ms,
                                 on_pre= """ 
                                 V_post = W_ring2inh 
                                 """
                                 )

        self.Inh2Ring = Synapses(self.inhibit_neuron, self.excitatory_neurons, model="W_inh2ring : volt", name="inh2ring_synapses",dt=2*ms, 
                                  on_pre="""
                                  V_post = W_inh2ring
                                  """
                                  )
 
        self.Inh2Inh = Synapses(self.inhibit_neuron, self.inhibit_neuron, model = "W_inh2inh : volt", name="inh2inh_synapses", dt=2*ms,
                                on_pre="""
                                V_post = W_inh2inh
                                """
                                )

        self.Ring2Ring = Synapses(self.excitatory_neurons, self.excitatory_neurons, model="W_ring2ring : volt", name="ring2rings_synapses",dt=2*ms,
                                  )
