...
class BoundedRingAttractor(RingAttractor):
    ...
    def __init__(self,
        ...
        # self.numNeurons=self.numNeurons
        # Store boundary info.
        self.limit_neuron = limit_neuron
        self.has_boundary = limit_neuron is not None
        if self.has_boundary:
            if not (0 <= limit_neuron < self.numNeurons):
                raise ValueError(f"boundary_neuron must be in [0, {self.numNeurons-1}], got {limit_neuron}")
            if limit_neuron==self.numNeurons-1:
                raise ValueError(f"boundary_neuron cannot be the last neuron (index {self.numNeurons-1}), at least one out of bound neuron is needed.")
            if limit_neuron==0:
            ...
            self.neuron_at_theta_star = int(self.theta_star / (2 * pi) * self.numNeurons)
            t_thresh_0=((self.numNeurons-self.neuron_at_theta_star)//4)*2*pi/self.numNeurons
            t_thresh_limit=((self.neuron_at_theta_star-self.limit_neuron)//4)*2*pi/self.numNeurons
            t_thresh_inbound=(self.limit_neuron//4)*2*pi/self.numNeurons
            ...
            self.ring_synapses_asym = Synapses(self.ring_pool, self.ring_pool,
                                    ...
                                    on_pre='I_vel_post += vel_in * int(vel_on)*(w_asym_positive * int(vel_in > 0) - int(vel_on) * w_asym_negative * int(vel_in < 0))')
            ...
            self.N_inboundNeurons = self.numNeurons
